...
uses), and each config is run both with OCR off and with OCR on so the OCR cost per
mode is visible. Reports per-document and aggregate throughput plus a profiler-based
stage breakdown for each run, then a side-by-side comparison. Runs in one of two modes
...
Environment variables:
  ...
  OUTPUT_DIR           Directory for the per-run results JSON (default: /out)
...
def _init_worker(warmup_pdf: str, config: ProcessingConfig, ocr: OcrEngine, barrier: Any = None) -> None:
...
def run_conversions(
    paths: list[str], warmup_pdf: str, config: ProcessingConfig, ocr: OcrEngine
...
def run_config(
    ...
    output_file = output_dir / f"benchmark_{name}{'' if ocr == 'none' else '_' + ocr}.json"
