"""OCR Helpers for Agentic Document Processing."""
...
def draw_bounding_boxes_2(groundings: dict, document_path: str | Path, base_path: str = ".") -> PILImage.Image | None:
    ...
    def create_annotated_image(image: PILImage.Image, groundings: dict, page_num: int = 0) -> PILImage.Image | None:
...
def draw_bounding_boxes(parse_response: Any, document_path: str | Path) -> None:
    ...
    def create_annotated_image(image: PILImage.Image, groundings: dict, page_num: int = 0) -> PILImage.Image:
        """Create an annotated image with grounding boxes and labels."""
        annotated_img = image.copy()
        draw = ImageDraw.Draw(annotated_img)
...
def create_cropped_chunk_images(
    ...
    first_page: int,
    doc_name: str,
) -> dict[str, dict[str, PILImage.Image]]:
    ...
    for field_name, metadata in extraction_metadata.items():
        ...
        field_images[field_name] = {"crop": cropped, "outlined": outlined}
...
def extract_chunk_image(
    ...
    if highlight:
        ...
        draw.rectangle(
            [padding, padding, chunk_img.width - padding - 1, chunk_img.height - padding - 1], outline="red", width=3
        )
